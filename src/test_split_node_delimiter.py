...
class TestTextNode(unittest.TestCase):
    def test_plain_text(self):
        ...
        node2 = TextNode("this is some other text without nested elements",
                        TextType.PLAIN)
        new_nodes = split_node_delimiter([node, node2], "**", TextType.BOLD)
    ...
    def test_italic_text(self):
        ...
        node2 = TextNode("this is some other text without nested elements",
                        TextType.PLAIN)
        new_nodes = split_node_delimiter([node, node2], "_", TextType.ITALIC)
        self.assertNotEqual(new_nodes, [node, node2])
        self.assertEqual(new_nodes[0].text, "this is some text with a nested ")
        self.assertEqual(new_nodes[0].text_type, TextType.PLAIN)
    ...
    def test_code_text(self):
        ...
        node2 = TextNode("this is some other text without nested elements",
                        TextType.PLAIN)
        new_nodes = split_node_delimiter([node, node2], "`", TextType.CODE)
        self.assertNotEqual(new_nodes, [node, node2])
        self.assertEqual(new_nodes[0].text, "this is some text with a nested ")
        self.assertEqual(new_nodes[0].text_type, TextType.PLAIN)
